...
{   
    ...
    return(
            ...
            <div className="flex flex-col grow w-1/4">
                <div ref={display} className='bg-amber-50 m-1 rounded grow border border-indigo-700 border-2 px-2 overflow-y-auto'> {/*display*/}
                    <ul className=" h-full flex flex-col  columns-1"> 
                                {stompClient==null&&(<h1 className="text-black text-center m-auto text-3xl">conectando...</h1>)}
                                {messages.map((msg,index)=>(    
                                    <li key={index} className={`text-black  w-fit max-w-48 break-words ${msg.senderName==sender.username ? 'self-end' : 'self-start'} py-1 px-2 rounded-2xl my-2 right-0 bg-indigo-500`}>
                                        <p className={`text-indigo-500 ${msg.senderName==sender.username&&'text-end'}`}>{msg.senderName==sender.username? '' : msg.senderName} </p> <p className="text-amber-50 text-left">{msg.content}</p>
                                    </li>
                                ))}
